    ...
    <div class="cart" v-if="this.$store.getters.getNum>0">
      <div class="cart-top">
        ...
        <div class="total">
          ...
          <el-button type="danger" round @click="goAccount">去结算<span
        ...
        <div class="list-item" v-for="(item, index) in products" :key="item.id">
          ...
          <div class="product-info" :class="item.out?'out':''">
            ...
            <div class="product-info-text">
              <p class="name-title"><span style="color: red;font-weight: bolder;font-size: 25px;"></span>{{item.productid}} - {{item.title}}
              </p>
...
<script>
  ...
  export default {
    ...
    mounted() {
      ...
      console.log("list-height:" + this.$refs.list.clientHeight);
    ...
    methods: {
      ...
      goAccount() {
        ...
        if (checkedNum > 0) {
          ...
          for (var i = 0; i < checkedNum; i++) {
            ...
            let idnumber = checkedItem[i].idnumber;
            let orderid = createtime + username + idnumber;
            ...
            this.$axios
              .post(this.$target + '/api/order/new', {
                ...
                addressid: 0,
                args: checkedItem[i].args.toString(),
              ...
              .then(res => {
                ...
                if (res.data.errno) {
                  ...
                  console.log("goAccount-res-id:" + res.data.data.id);
      ...
      goConfirm(id) {
        ...
        idList.unshift(id);
        this.idList = idList;
        if (idList.length == this.$store.getters.getCheckedNum) {
          this.$router.push({
            path: '/orderConfirm',
            query: {
              idList: idList
